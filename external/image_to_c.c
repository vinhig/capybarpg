...
const char *szType[] = {"Unknown",  "PNG",     "JFIF", "Win BMP",
                        "OS/2 BMP", "TIFF",    "GIF",  "Portable Pixmap",
                        "Targa",    "JEDMICS", "CALS", "PCX"};
const char *szComp[] = {"Unknown",
                        "Flate",
                        "JPEG",
                        "None",
                        "RLE",
                        ...
                        "Thunderscan RLE",
                        "JBIG (T.85)"};
const char *szPhotometric[] = {
    "WhiteIsZero",       "BlackIsZero", "RGB",   "Palette Color",
...
int ImageInfo(FILE *iHandle, int iFileSize, char *szInfo, int *iDataOff) {
  ...
  else if ((cBuf[0] == 'I' && cBuf[1] == 'I') ||
           (cBuf[0] == 'M' && cBuf[1] == 'M'))
    iFileType = FILETYPE_TIFF;
  else {
    i = MOTOLONG(cBuf) & 0xffff8080;
    if (i == 0x50360000 || i == 0x50350000 ||
        i == 0x50340000) // Portable bitmap/graymap/pixmap
  ...
  if (MOTOLONG(cBuf) != 0x1ba && MOTOLONG(cBuf) != 0x1b3 && i == 0 &&
      (j == 1 || j == 2 || j == 3 || j == 9 || j == 10 || j == 11))
  ...
  case FILETYPE_BMP:
    ...
    *iDataOff = cBuf[10]; // OffBits
    ...
    if (cBuf[30] &&
        (iBpp == 4 || iBpp == 8))  // if biCompression is non-zero (2=4bit rle,
                                   // 1=8bit rle,4=24bit rle)
  ...
  case FILETYPE_OS2BMP:
    ...
    if (iHeight & 0x8000) // upside down
      iHeight = 65536 - iHeight;
    if (cBuf[30] == 1 || cBuf[30] == 2 ||
        cBuf[30] == 4) // if biCompression is non-zero (2=4bit rle, 1=8bit
                       // rle,4=24bit rle)
      iCompression = COMPTYPE_RLE; // windows run-length
    break;
  ...
  case FILETYPE_CALS:
    ...
    } else // type 2
    {
      ...
      if (MOTOLONG(cBuf) == 0x7270656c &&
          MOTOLONG(&cBuf[4]) == 0x636e743a) // "rpelcnt:"
      {
        i = 9;
        iWidth = ParseNumber(cBuf, &i, 256);
        iHeight = ParseNumber(cBuf, &i, 256);
  ...
  case FILETYPE_JPEG:
    ...
    while (i < 32 && iMarker != 0xffc0 && j < iFileSize) {
      ...
      if (iMarker < 0xff00) // invalid marker, could be generated by "Arles
                            // Image Web Page Creator" or Accusoft
      {
        i += 2;
        continue; // skip 2 bytes and try to resync
      }
      if (iMarker == 0xffe0 && cBuf[i + 4] == 'E' &&
          cBuf[i + 5] == 'x') // EXIF, check for thumbnail
      {
        ...
        //               iOff = PILTIFFLONG(&cTemp[i+14], bMotorola); // get
        //               offset to first IFD (info) PILTIFFMiniInfo(pFile,
        //               bMotorola, j + 10 + iOff, TRUE);
      }
    ...
    if (iMarker != 0xffc0)
      ...
      iBpp =
          iBpp * cBuf[i + 9]; /* Bpp = number of components * bits per sample */
      ucSubSample = cBuf[i + 11];
      sprintf(szOptions, ", color subsampling = %d:%d", (ucSubSample >> 4),
              (ucSubSample & 0xf));
    }
    break;
  case FILETYPE_GIF:
  ...
  case FILETYPE_TIFF:
    ...
    for (i = 0; i < j; i++) // search tags for the info we care about
    {
      ...
      case 258:                                           // bits per sample
        ...
        else // need to read the first value from the list (they should all be
             // equal)
        {
          k = TIFFLONG(&cBuf[iOffset + 8], bMotorola);
          if (k < iFileSize) {
            fseek(iHandle, k, SEEK_SET);
            iBytes = fread(&cBuf[iOffset], 1, 2,
                           iHandle); // okay to overwrite the value we just used
      ...
      //            sprintf(szOptions, ", Photometric = %s, Planar config = %s",
    //            szPhotometric[iPhotoMetric], szPlanar[iPlanar]); break;
  } // switch
...
int main(int argc, char *argv[]) {
  ...
  if (bStrip && iDataOff != iDataSize) {
    iStart = iDataOff;
    printf("//\n// This hex data is only the image; the header + metadata has "
           "been removed\n");
    ...
    printf("//\n// %s\n// Data size = %d bytes\n//\n", szLeaf,
           iSize); // comment header with filename
  }
  if (szInfo[0])
    printf("%s", szInfo);
  ...
  printf("const unsigned %s_size = %d;\n", szLeaf, iSize);
  printf("const unsigned char %s[] PROGMEM = {\n",
